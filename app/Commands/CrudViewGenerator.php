...
class CrudViewGenerator extends BaseCommand
{
    ...
    private function getFieldData($tableName)
    {
        ...
        $formContent   = [];
        $columnItem    = [];
        $ignoredFields = ['id', 'created_at', 'updated_at', 'created_by', 'updated_by'];
    ...
    private function generateFormInput($field, $objForeignKey)
    {
        ...
        switch ($type) {
            case 'text':
                $formInput = <<<FIELD
                    <?= form_textarea('{$name}', old('{$name}', \$data->{$name} ?? ''), "rows='4' class='{$classInput}' {$required} placeholder='".lang('crud.{$this->tableName}.{$name}')."' ") ?>
                    FIELD;
            break;
            ...
            case 'select2':
                ...
                $formInput   = <<<FIELD
                    <?= form_dropdown('{$name}',\${$optionItems} ,old('{$name}', \$data->{$name} ?? ''), "class='{$classInput}' {$required} placeholder='".lang('crud.{$this->tableName}.{$name}')."' ") ?>
                    FIELD;
            break;

            default:
                $formInput = <<<FIELD
                    <?= form_input('{$name}', old('{$name}', \$data->{$name} ?? ''), "class='{$classInput}' {$required} placeholder='".lang('crud.{$this->tableName}.{$name}')."' ") ?>
                    FIELD;
        }

        return <<<FIELD
                            <div class="row mb-3">
                                <?= form_label(lang('crud.{$this->tableName}.{$name}'),'',['for' => '{$name}', 'class' => 'col-form-label col-sm-2']) ?>
                                <div class="col-sm-10">
                                    {$formInput}
