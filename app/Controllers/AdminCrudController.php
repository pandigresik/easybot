...
class AdminCrudController extends AdminController
{
    ...
    public function create()
    {
        ...
        return redirect()->to(url_to($this->getBaseController()))->with('message', lang('app.resourceSaved', [$this->langModel]));
    }
    ...
    {        
        ...
        if ($this->model->db->affectedRows() === 0) {
            if ($this->isHxRequest()) {
                Services::session()->setFlashdata('error', lang('app.resourceNotFound'));

                return '<aside id="alerts-wrapper" hx-swap-oob="true">{alerts}</aside>';
            }

            return redirect()->back()->with('error', lang('app.unknownError'));
        }
        $this->writeLog();
        if ($this->isHxRequest()) {
            Services::session()->setFlashdata('message', lang('app.resourceDeleted', [$this->langModel]));            
            return '<aside id="alerts-wrapper" hx-swap-oob="true">{alerts}</aside>';
        }

        return redirect()->back()->with('message', lang('app.resourceDeleted', [$this->langModel]));
    }    
    ...
    {
        return [
            'actionUrl' => $id ? route_to($this->getBaseController(), $id) : route_to($this->getBaseController()),
            'backUrl'   => route_to($this->getBaseController()),
        ];
    }
